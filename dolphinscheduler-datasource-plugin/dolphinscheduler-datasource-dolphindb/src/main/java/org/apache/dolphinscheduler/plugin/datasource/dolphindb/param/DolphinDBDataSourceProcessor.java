...
public class DolphinDBDataSourceProcessor extends AbstractDataSourceProcessor {
    ...
    public BaseDataSourceParamDTO createDatasourceParamDTO(String connectionJson) {
        ...
        dolphinDBDataSourceParamDTO.setOther(parseOther(connectionParams.getOther()));
    ...
    public ConnectionParam createConnectionParams(BaseDataSourceParamDTO datasourceParam) {
        ...
        String address = String.format("%s%s:%s", DataSourceConstants.JDBC_DOLPHINDB,dolphinDBParam.getHost(),dolphinDBParam.getPort());
        ...
        dolphinDBConnectionParam.setOther(transformOther(dolphinDBParam.getOther()));
        ...
        dolphinDBConnectionParam.setProps(dolphinDBParam.getOther());
    ...
    public String getJdbcUrl(ConnectionParam connectionParam) {
        ...
        if(!StringUtils.isEmpty(dolphinDBConnectionParam.getOther())){
            return String.format("%s?%s",jdbcUrl,dolphinDBConnectionParam.getOther());
        }
        return jdbcUrl;
    }
    ...
    public Connection getConnection(ConnectionParam connectionParam) throws ClassNotFoundException, SQLException, IOException {
        ...
        if(!StringUtils.isEmpty(dolphinDBConnectionParam.getUser()) && !StringUtils.isEmpty(dolphinDBConnectionParam.getPassword()))
            return DriverManager.getConnection(getJdbcUrl(connectionParam),dolphinDBConnectionParam.getUser(),PasswordUtils.decodePassword(dolphinDBConnectionParam.getPassword()));
        else
    ...
    public String getDatasourceUniqueId(ConnectionParam connectionParam, DbType dbType) {
        ...
        return MessageFormat.format("{0}@{1}@{2}@{3}@{4}", dbType.getDescp(), baseConnectionParam.getUser(),
                PasswordUtils.encodePassword(baseConnectionParam.getPassword()), baseConnectionParam.getJdbcUrl(),baseConnectionParam.getOther());
    }
